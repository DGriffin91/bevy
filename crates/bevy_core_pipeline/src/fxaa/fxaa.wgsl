...
fn fs_main(in: FullscreenVertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    let centerSample = textureSample(screenTexture, samp, texCoord);
    ...
    let lumaDown = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(0, -1)).rgb);
    let lumaUp = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(0, 1)).rgb);
    let lumaLeft = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(-1, 0)).rgb);
    let lumaRight = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(1, 0)).rgb);
    ...
    //     fragColor = vec4<f32>(colorCenter, 1.0);
    //     return fragColor;
    // }

    // Query the 4 remaining corners lumas.
    let lumaDownLeft  = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(-1, -1)).rgb);
    let lumaUpRight   = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(1, 1)).rgb);
    let lumaUpLeft    = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(-1, 1)).rgb);
    let lumaDownRight = rgb2luma(textureSample(screenTexture, samp, texCoord, vec2<i32>(1, -1)).rgb);
    ...
    var stepLength = inverseScreenSize.x;
    if (isHorizontal) {
    ...
    var lumaEnd1 = rgb2luma(textureSample(screenTexture, samp, uv1).rgb);
    var lumaEnd2 = rgb2luma(textureSample(screenTexture, samp, uv2).rgb);
    ...
    if (!reached1) {
        uv1 = uv1 - offset; // * QUALITY(1); // (quality 1 is 1.0)
    }
    if (!reached2) {
        uv2 = uv2 + offset; // * QUALITY(1); // (quality 1 is 1.0)
    }

    // If both sides have not been reached, continue to explore.
    //if (!reachedBoth) { // TODO uniform control flow
        for (var i: i32 = 2; i < ITERATIONS; i = i + 1) {
            // If needed, read luma in 1st direction, compute delta.
            let s = rgb2luma(textureSample(screenTexture, samp, uv1).rgb); // TODO uniform control flow
            if (!reached1){ 
                lumaEnd1 = s;
                lumaEnd1 = lumaEnd1 - lumaLocalAverage;
            }
            // If needed, read luma in opposite direction, compute delta.
            let s = rgb2luma(textureSample(screenTexture, samp, uv2).rgb); // TODO uniform control flow
            if (!reached2){ 
                lumaEnd2 = s;
                lumaEnd2 = lumaEnd2 - lumaLocalAverage;
            }
            ...
            // if (reachedBoth){ break; } //TODO uniform control flow
        }
    //}

    // Compute the distances to each side edge of the edge (!).
    var distance1 = texCoord.y - uv1.y;
    var distance2 = uv2.y - texCoord.y;
    if (isHorizontal)  {
         distance1 = texCoord.x - uv1.x;
         distance2 = uv2.x - texCoord.x;
    };
    ...
    var correctVariation = correctVariation2;
    if (isDirection1) {
    ...
    let finalColor = textureSample(screenTexture, samp, finalUv).rgb;
    let fragColor = vec4<f32>(finalColor, centerSample.a);
    return fragColor;
}
