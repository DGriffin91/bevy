...
fn main() {
    App::new()
        ...
        .add_systems(Update, update_state) //move_camera
        .run();
}
...
fn update_state(
    ...
    if keys.just_pressed(KeyCode::Key3) {
        prepass_string = Some("Forward + Prepass");
    ...
    *frame_time_ms = (diagnostics
    ...
    for mut text in &mut query {
        ...
        if let Some(prepass_string) = prepass_string.as_ref() {
            text.sections[1].value = prepass_string.to_string();
        }
